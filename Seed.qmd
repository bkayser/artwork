...
for (i in 1:nrow(master)) {
  ...
  safe_owner <- file.path("Works", clean_name(rec_owner))
  ...
  if (!dir.exists(target_dir)) {
    ...
    message("Target directory not found: ", target_dir)
      ...
      # master$target_dir[i] <- as.character(target_dir)
      # fsid <- master$ID.FS[i] |> as.integer()
      # if (!is.na(fsid)) {
      #     to_copy <- filter(image_works, ID.FS == fsid)
      #     if (nrow(to_copy) > 0) {
      #         for (j in 1:nrow(to_copy)) {
      #             if (!file.exists(file.path(target_dir, basename(to_copy$image[j])))) {
      #                 file.copy(to_copy$image[j], target_dir)
      #                 message("Copy to ", target_dir, ", ", to_copy$image[j])
      #             }
      #         }
      #     }
      # }
...
.master <- master |>
    mutate(first = str_extract(Artist, ", ?(.*)$", group=1),
           last = str_extract(Artist, "^([^,]+),", group=1),
           `Artist Name` = ifelse(is.na(first), Artist, str_c(first, " ", last))) |>
    select(-first, -last)
    
write_lines(.master$`Artist Name` |> sort() |> unique(), file = "artist_names.txt")

master$Links |> na.omit() |> sort() |> unique() |>
    write_lines("links.txt")

deploy_info <- select(.master, -ID.FS, -ID.BK, -Value.FS, -Value.Safeco) |> 
    rename(Value = Value.DRK) |>
    left_join(read_csv("artists.csv")) |>
    select(ID,
           `Artist Name`,
           Born = `Year of Birth`,
           Died = `Year of Death`,
...
write_csv(deploy_info, "deploy_info.csv", na="\n")
